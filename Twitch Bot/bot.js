...
function onMessageHandler (target, context, msg, self) {
    ...
    if (commandName === '!d20') {
        ...
        if (person === "kumarnt") {
            client.say(target, "On behalf of my owner, I would like to welcome you all to the DOGE SQUAD!!");
        } else if(person === 'smolceci') {
            client.say(target, "HI MOM! I LOVE YOU, and I miss you too :3");
        } else if(person === 'darkwing1203') {
            client.say(target, "Hey Brady, hope you brought me snacks to bite while watching this stream! Woof");
        } else if(person === 'rnjrgreen') {
            client.say(target, "We may play Anubis, but that does not mean we are 'a noob'... jk welcome to the stream Mr.Green");
        } else if(person === 'dragongaymerwaz') {
            client.say(target, "Greetings Mr.Waz I hope you Spheal welcome in the stream X)");
        } else {
            client.say(target, `Greetings ${name}, I am Jaddy! A fusion of Jason and Diddy! I am the Ultimate Doge!!`);
        }
        console.log(`* Executed ${commandName} command`);

    } else if (commandName === '!lurk') {
        client.say(target, `Thanks for lurking ${name}, my owner really appreciates you deciding to lurk this channel! Woof`);
        console.log(`* Executed ${commandName} command`);
    //QUEUE COMMANDS
        ...
        if(!queue.includes(name)) {
            ...
            if(index == -1) {
                ...
                client.say('Aborting Operation, Something went wrong, please inform owner or ERROR fsrii-1a119');
                return;
            } else {
